...
public class TwitterKafkaConsumer implements KafkaConsumer<TwitterAvroModel> {
    ...
    public TwitterKafkaConsumer(KafkaListenerEndpointRegistry listenerEndpointRegistry,
        ...
        LOG.info("Creating TwitterKafkaConsumer...");
        LOG.info("configData:schemaRegistryURL " + configData.getSchemaRegistryUrl());
        if(indexClient!=null)
        {
            LOG.info("TwitterKafkaConsumer constructor: Elastic Client: " + indexClient.toString());
        } else {
            LOG.info("indexClient === NULL !!!!!");
        }
